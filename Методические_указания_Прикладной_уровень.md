...
Скорее всего в своем проекте вы будете исполльзовать git для контроля версий своего проекта. Небольшой совет по
использованию гита:
...
wss.on('connection', (websocketConnection: WebSocket, req: Request) => {
  ...
  websocketConnection.on('message', (messageString: string) => {
    ...
    const message: MessageCard = JSON.parse(messageString)
